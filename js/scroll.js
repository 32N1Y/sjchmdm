    ...
    if (window.Modernizr) {
        ...
        t.horizonMoving = function(t, i, n) {
            ...
            if (n = n || tps.bestTranslateType, "" == t || "left" != n) {
                ...
                if($(i).attr('id') == 'parallax'){
                	t < (-600) ? $('.nav').fadeIn(300) : $('.nav').fadeOut(300)
                	var index = Math.abs(Math.ceil((t-10)/1920));
                	$('.menu_item').eq(index).addClass('active').siblings().removeClass('active');
                }
                i.css({
                    WebkitTransform: e,
                    MozTransform: e,
